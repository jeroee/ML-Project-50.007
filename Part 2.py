...
def emissionMatrix(df, emission_matrix):
    ...
    df_merged['Probability'] = df_merged.word_count/df_merged.state_count    # get emission probability (count of word in that state/ state count)
...
def save_df(df,path):
    np.savetxt(path, df.values, fmt='%s')
